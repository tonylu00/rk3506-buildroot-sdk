...
make_root_fs() {
    ...
    if [ "$root_size_byte" -le $((2 * 1024 * 1024)) ]; then
        ...
        if [ "$fat_size_byte" -gt "$root_size_byte" ]; then
            echo "Error: fat filesystem is $fat_size_byte greater than root partition size $root_size_byte"
    ...
    if [ "$root_file_size" -gt "$root_size_byte" ]; then
        echo "Error: fat filesystem is greater than root partition size $root_size_byte"
        echo "Please enlarge root partition size"
