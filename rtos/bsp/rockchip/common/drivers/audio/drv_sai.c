...
static int rk_sai_pm_suspend(const struct rt_device *device)
{
    return RT_EOK;
}

static void rk_sai_pm_resume(const struct rt_device *device)
{
}
...
static struct audio_dai *rk_sai_init_dai(struct HAL_SAI_DEV *hSai)
{
    ...
    rt_pm_register_device(&sai->parent, &rk_sai_pm_ops);
#endif
