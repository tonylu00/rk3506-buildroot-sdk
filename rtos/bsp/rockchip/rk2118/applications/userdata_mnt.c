...
static int erase_userdata(struct rt_flash_partition *info)
{
    int len;
...
do_mount:
    ...
    rt_kprintf("userdata_mnt: try mount\n");
    if (dfs_mount(USERDATA_PART_NAME, USERDATA_MOUNT_POINT, "elm", 0, 0) != 0)
    {
        rt_kprintf("mount %s fs[elm] on %s failed.\n", USERDATA_PART_NAME, USERDATA_MOUNT_POINT);
        if (mount_count-- <= 0)
            return;
        ...
        if (erase_userdata(&udata_info) != RT_EOK)
        {
        ...
        // 3. mkfat on userdata
        rt_kprintf("userdata_mnt: mkfat on userdata\n");
        if (dfs_mkfs("elm", USERDATA_PART_NAME) != RT_EOK)
        {
