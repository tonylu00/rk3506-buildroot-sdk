...
extern int rk_ota_get_flash_size(char *path, DWORD *flash_size, DWORD *block_num);
...
rt_bool_t rk_ota_download_loader(unsigned char *data_buf, int size, char *dest_path)
{
    ...
    if (rk_ota_get_flash_info(NULL, &uiFlashBlockSize, &uiFlashPageSize) != 0)
    {
        RK_OTA_ERR("%s-%d: get mtd info error\n", __func__, __LINE__);
        ret = RT_FALSE;
        goto dw_loader_out;
    }
