...
static int analyticRKFWImage(const char *filepath, PRKIMAGE_HDR phdr, DWORD offset, DWORD fw_size)
{
    ...
    if (rkimage_head.emSupportChip[0] != 0x38 || rkimage_head.emSupportChip[1] != 0x31 ||
            rkimage_head.emSupportChip[2] != 0x31 || rkimage_head.emSupportChip[3] != 0x32)
    {
        RK_OTA_ERR("chip type in FW is not 2118\n");
        //close(fd);
        return -2;
    }
