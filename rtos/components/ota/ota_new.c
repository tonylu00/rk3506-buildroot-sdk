...
static rt_bool_t ota_flash_device_open(void)
{
#ifdef RT_USING_SNOR
    if (snor_dev == RT_NULL)
    {
        ...
        if (snor_dev == RT_NULL)
        {
            RK_OTA_ERR("%s: rt_device_find snor failed\n", __func__);
            return RT_FALSE;
        }
    }
    return RT_TRUE;
    ...
    if (snand_dev == RT_NULL)
    {
        ...
        if (snand_dev == RT_NULL)
        {
            RK_OTA_ERR("%s: rt_device_find snand failed\n", __func__);
            return RT_FALSE;
        }
    }
    return RT_TRUE;
...
#elif defined(RT_USING_SPINAND)
int get_snand_block_size(struct rt_mtd_nand_device *dev)
{
    if (dev != RT_NULL)
        return dev->page_size * dev->pages_per_block;
    return 0;
}
...
int rk_ota_get_flash_info(void *dev, DWORD *block_size, DWORD *page_size)
{
    if (snand_dev != RT_NULL)
    {
        *block_size = snand_dev->page_size * snand_dev->pages_per_block;
        *page_size = snand_dev->page_size;
        return 0;
    }
...
int rk_ota_get_flash_size(char *path, DWORD *flash_size, DWORD *block_num)
{
    DWORD total_size;

    total_size = snand_dev->page_size * snand_dev->pages_per_block * snand_dev->block_total;

    if (flash_size)
    {
        *flash_size = total_size / 1024; //Kib
        RK_OTA_INF("%s: flash total size %d\n", __func__, total_size);
    }
    if (block_num)
    {
        *block_num = (total_size / 1024) * 2;
        RK_OTA_INF("%s: block num %d\n", __func__, *block_num);
    }
...
{
#ifdef RT_USING_SNOR
    if (misc->size < 2 * snor_dev->block_size)
    {
    ...
    return RT_TRUE;
#elif defined(RT_USING_SPINAND)
    if (misc->size < 2 * get_snand_block_size(snand_dev))
    {
        RK_OTA_ERR("%s: misc size %d too small\n", __func__, misc->size);
        return RT_FALSE;
    }
...
int rk_ota_get_dfs_part_info(void)
{
    ...
    if (ota_flash_device_open() != RT_TRUE)
    {
        ret = OTA_STATUS_ERR;
    ...
    RK_OTA_INF("snand block_size %d\n", get_snand_block_size(snand_dev));
    if (snand_dev->page_size != 2048 && snand_dev->page_size != 4096)
    {
        RK_OTA_ERR("%s: snand error page_size %d\n", __func__, snand_dev->page_size);
        ret = OTA_STATUS_ERR;
        goto get_dfs_part_out;
    }

    if (get_snand_block_size(snand_dev) > 256 * 1024)
    {
    ...
    {
#ifdef RT_USING_SNOR
        if (strcmp(dfs_part_info[i].name, "userdata") && dfs_part_info[i].size % snor_dev->block_size)
        {
            RK_OTA_ERR("%s: part %s size 0x%x not 0x%x align\n", __func__,
                       dfs_part_info[i].name, dfs_part_info[i].size, snor_dev->block_size);
            ret = OTA_STATUS_ERR;
            goto get_dfs_part_out;
        }
#elif defined(RT_USING_SPINAND)
        if (strcmp(dfs_part_info[i].name, "userdata") && dfs_part_info[i].size % get_snand_block_size(snand_dev))
        {
            RK_OTA_ERR("%s: part %s size 0x%x not 0x%x align\n", __func__,
                       dfs_part_info[i].name, dfs_part_info[i].size, get_snand_block_size(snand_dev));
            ret = OTA_STATUS_ERR;
            goto get_dfs_part_out;
        }
#endif
        if (!strcmp(dfs_part_info[i].name, "cpu0_secure_b"))
        {
            ab++;
        }

        if (!strcmp(dfs_part_info[i].name, "cpu0_os_b"))
        {
            ab++;
        }
...
#ifndef RT_USING_NEW_OTA_MODE_A_RECOVERY
    if (ab != 2)
    {
        RK_OTA_ERR("%s: no ab part\n", __func__);
        ret = OTA_STATUS_ERR;
        goto get_dfs_part_out;
    }
#endif
...
int rk_ota_write_misc_recovery_info(struct misc_recovery_info *info)
{
    ...
    block_size = snor_dev->block_size;
    write_size = block_size;
#elif defined(RT_USING_SPINAND)
    write_size = RK_OTA_INFO_SIZE;
    block_size = get_snand_block_size(snand_dev);
...
int rk_ota_part_info_chk(char *url)
{
    ...
    for (i = 0; i < fw_part_num; i++)
    {
        ...
        {
            fw_rk_part_info[i].size = snor_dev->block_end * snor_dev->block_size - fw_rk_part_info[i].offset;
        }
        else
        {
    ...
    for (i = 0; i < part_num; i++)
    {
        ...
        if (!strcmp(fw_rk_part_info[i].name, "userdata"))
            continue;
...
int rk_ota_upgrade_part(int i, rt_uint32_t offset, rt_uint32_t size)
{
    ...
    while (size1)
    {
        ...
        readed = size1 > erase_size ? erase_size : snor_dev->block_size;
#elif defined(RT_USING_SPINAND)
        if (snand_dev == RT_NULL)
        {
            RK_OTA_ERR("%s: snand_dev = null\n", __func__);
            return OTA_STATUS_ERR;
        }
        readed = get_snand_block_size(snand_dev);
#endif
...
int rk_ota_upgrade_fw(void)
{
    ...
    for (i = 0; i < dfs_part_num; i++)
    {
        ...
        if (!strcmp(dfs_part_info[i].name, "userdata") ||
