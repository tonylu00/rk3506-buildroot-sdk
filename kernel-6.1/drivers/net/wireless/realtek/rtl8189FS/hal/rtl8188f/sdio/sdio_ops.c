...
static void ReadInterrupt8188FSdio(PADAPTER adapter, u32 *ret)
{
	u32 hisr;

#if CMD52_ACCESS_HISR_RX_REQ_LEN
	u32 himr = GET_HAL_DATA(adapter)->sdio_himr;
	u8 val8, hisr_len;
	int i;
	...
	for (i = 0; i < hisr_len; i++) {
		val8 = SdioLocalCmd52Read1Byte(adapter, SDIO_REG_HISR + i);
		hisr |= (val8 << (8 * i));
	}
#else
	hisr = SdioLocalCmd53Read4Byte(adapter, SDIO_REG_HISR);
#endif

	*ret = hisr;
}

void ClearInterrupt8188FSdio(PADAPTER adapter)
{
	u32 hisr = GET_HAL_DATA(adapter)->sdio_hisr & MASK_SDIO_HISR_CLEAR;

	if (hisr) {
		#if CMD52_ACCESS_HISR_RX_REQ_LEN
		SdioLocalCmd52Write4Byte(adapter, SDIO_REG_HISR, hisr);
		#else
		hisr = cpu_to_le32(hisr);
		sdio_local_write(adapter, SDIO_REG_HISR, 4, (u8 *)&hisr);
		#endif
	}
}

static void ReadRxReqLen8188FSdio(PADAPTER adapter, u16 *ret)
{
	u16 rx_req_len;
	...
	_sdio_local_read(adapter, SDIO_REG_RX0_REQ_LEN, 2, data);
	rx_req_len = le16_to_cpu(*(u16 *)data);
#endif
...
void sd_recv(_adapter *adapter)
{
	...
	do {
		if (phal->SdioRxFIFOSize == 0) {
			ReadRxReqLen8188FSdio(adapter, &phal->SdioRxFIFOSize);
			if (phal->SdioRxFIFOSize == 0)
				break;
		}
...
void sd_int_hdl(PADAPTER padapter)
{
	...
	if (phal->sdio_hisr & phal->sdio_himr) {
		...
		ClearInterrupt8188FSdio(padapter);
...
#ifndef CMD53_8_ACCESS_TX_AVAL_PAGE
#define CMD53_8_ACCESS_TX_AVAL_PAGE 0
#endif

#ifndef DBG_ACCESS_TX_AVAL_PAGE
#define DBG_ACCESS_TX_AVAL_PAGE 1
#endif
...
u8 HalQueryTxBufferStatus8188FSdio(PADAPTER padapter)
{
	...
	systime query_start = rtw_get_current_time();
	u8 NumOfFreePage[8];
	bool valid;
	/* _irqL irql; */
...
query:
	...
	#if CMD53_8_ACCESS_TX_AVAL_PAGE
	/* sdio local cmd53 8-byte read */
	sdio_local_read(padapter, SDIO_REG_FREE_TXPG, 8, NumOfFreePage);
	#else
	sdio_local_read(padapter, SDIO_REG_FREE_TXPG, 4, NumOfFreePage);
	sdio_local_read(padapter, SDIO_REG_FREE_TXPG + 4, 4, NumOfFreePage + 4);
	#endif

	valid = NumOfFreePage[0] <= phal->tx_high_page
		&& NumOfFreePage[2] <= phal->tx_normal_page
		&& NumOfFreePage[4] <= phal->tx_low_page
		&& NumOfFreePage[6] <= phal->tx_pub_page;
	...
	if (valid) {
		...
		/* invalid value from sdio local cmd53 8-byte read */
		#if DBG_ACCESS_TX_AVAL_PAGE
		{
			u8 mac_fifo_page_cmd53_8[8], mac_rqpn_npq_cmd53_8[8];
			u32 mac_fifo_page_cmd53_4, mac_rqpn_npq_cmd53_4;
			u8 mac_fifo_page_cmd52[4], mac_rqpn_npq_cmd52[4];
			u8 sd20_cmd53[8];
			u32 sd20_cmd52, sd24_cmd52;
			u8 page_num[SDIO_MAX_TX_QUEUE];
			int i;

			#if CMD53_8_ACCESS_TX_AVAL_PAGE
			/* sdio local cmd53 4-byte read */
			sdio_local_read(padapter, SDIO_REG_FREE_TXPG, 4, sd20_cmd53);
			sdio_local_read(padapter, SDIO_REG_FREE_TXPG + 4, 4, sd20_cmd53 + 4);
			#else
			/* sdio local cmd53 8-byte read */
			sdio_local_read(padapter, SDIO_REG_FREE_TXPG, 8, sd20_cmd53);
			#endif

			/* sdio local cmd52 read */
			sd20_cmd52 = SdioLocalCmd52Read4Byte(padapter, SDIO_REG_FREE_TXPG);
			sd24_cmd52 = SdioLocalCmd52Read4Byte(padapter, SDIO_REG_FREE_TXPG + 4);

			/* mac cmd53 8-byte read */
			rtw_read_mem(padapter, REG_FIFOPAGE, 8, mac_fifo_page_cmd53_8);
			rtw_read_mem(padapter, REG_RQPN_NPQ, 8, mac_rqpn_npq_cmd53_8);

			/* mac cmd53 4-byte read */
			mac_fifo_page_cmd53_4 = rtw_read32(padapter, REG_FIFOPAGE);
			mac_rqpn_npq_cmd53_4 = rtw_read32(padapter, REG_RQPN_NPQ);

			/* mac cmd52 read */
			for (i = 0; i < 4; i++)
				mac_fifo_page_cmd52[i] = rtw_read8(padapter, REG_FIFOPAGE + i);
			for (i = 0; i < 4; i++)
				mac_rqpn_npq_cmd52[i] = rtw_read8(padapter, REG_RQPN_NPQ + i);

			RTW_WARN("%s (cmd53_%d),     cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, CMD53_8_ACCESS_TX_AVAL_PAGE ? 8 : 4
				, NumOfFreePage[0], NumOfFreePage[2], NumOfFreePage[4], NumOfFreePage[6]);

			page_num[HI_QUEUE_IDX] = sd20_cmd53[0];
			page_num[MID_QUEUE_IDX] = sd20_cmd53[2];
			page_num[LOW_QUEUE_IDX] = sd20_cmd53[4];
			page_num[PUBLIC_QUEUE_IDX] = sd20_cmd53[6];
			RTW_WARN("%s (cmd53_%d),     cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, CMD53_8_ACCESS_TX_AVAL_PAGE ? 4 : 8
				, page_num[HI_QUEUE_IDX], page_num[MID_QUEUE_IDX], page_num[LOW_QUEUE_IDX], page_num[PUBLIC_QUEUE_IDX]);

			page_num[HI_QUEUE_IDX] = sd20_cmd52 & 0x000000FF;
			page_num[MID_QUEUE_IDX] = (sd20_cmd52 & 0x00FF0000) >> 16;
			page_num[LOW_QUEUE_IDX] = sd24_cmd52 & 0x000000FF;
			page_num[PUBLIC_QUEUE_IDX] = (sd24_cmd52 & 0x00FF0000) >> 16;
			RTW_WARN("%s (cmd52),       cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, page_num[HI_QUEUE_IDX], page_num[MID_QUEUE_IDX], page_num[LOW_QUEUE_IDX], page_num[PUBLIC_QUEUE_IDX]);

			page_num[HI_QUEUE_IDX] = mac_fifo_page_cmd53_8[0];
			page_num[MID_QUEUE_IDX] = mac_rqpn_npq_cmd53_8[1];
			page_num[LOW_QUEUE_IDX] = mac_fifo_page_cmd53_8[1];
			page_num[PUBLIC_QUEUE_IDX] = mac_fifo_page_cmd53_8[2];
			RTW_WARN("%s (mac cmd53_8), cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, page_num[HI_QUEUE_IDX], page_num[MID_QUEUE_IDX], page_num[LOW_QUEUE_IDX], page_num[PUBLIC_QUEUE_IDX]);

			page_num[HI_QUEUE_IDX] = mac_fifo_page_cmd53_4 & 0x000000FF;
			page_num[MID_QUEUE_IDX] = (mac_rqpn_npq_cmd53_4 & 0x0000FF00) >> 8;
			page_num[LOW_QUEUE_IDX] = (mac_fifo_page_cmd53_4 & 0x0000FF00) >> 8;
			page_num[PUBLIC_QUEUE_IDX] = (mac_fifo_page_cmd53_4 & 0x00FF0000) >> 16;
			RTW_WARN("%s (mac cmd53_4), cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, page_num[HI_QUEUE_IDX], page_num[MID_QUEUE_IDX], page_num[LOW_QUEUE_IDX], page_num[PUBLIC_QUEUE_IDX]);

			page_num[HI_QUEUE_IDX] = mac_fifo_page_cmd52[0];
			page_num[MID_QUEUE_IDX] = mac_rqpn_npq_cmd52[1];
			page_num[LOW_QUEUE_IDX] = mac_fifo_page_cmd52[1];
			page_num[PUBLIC_QUEUE_IDX] = mac_fifo_page_cmd52[2];
			RTW_WARN("%s (mac cmd52),   cur H:%u, M:%u, L:%u, P:%u\n", __func__
				, page_num[HI_QUEUE_IDX], page_num[MID_QUEUE_IDX], page_num[LOW_QUEUE_IDX], page_num[PUBLIC_QUEUE_IDX]);
		}
		#endif

		if (rtw_get_passing_time_ms(query_start) < 2000) {
			rtw_msleep_os(10);
			goto query;
		} else
			RTW_WARN("%s invalid page number H:%u, M:%u, L:%u, P:%u\n", __func__
				, NumOfFreePage[0], NumOfFreePage[2], NumOfFreePage[4], NumOfFreePage[6]);
	}
