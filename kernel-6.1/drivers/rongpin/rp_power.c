...
static int boot_completed_flag = 0;
...
static int rp_wakeup_open(struct inode *inode, struct file *file)
{
       return 0;
}


static ssize_t rp_wakeup_write(struct file *file, const char *buffer,size_t count, loff_t *data)
{
       
                       input_event(gpio_data->input, EV_KEY, KEY_POWER, 1);
                       input_event(gpio_data->input, EV_KEY, KEY_POWER, 0);
                       input_sync(gpio_data->input);
       return count;
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0))
static const struct proc_ops rp_wakeup_ops = {
	.proc_open           = rp_wakeup_open,
	.proc_write          = rp_wakeup_write,	
	.proc_read           = NULL,
};
#else
static const struct file_operations rp_wakeup_ops = {
	.owner          = THIS_MODULE,
    .open           = rp_wakeup_open,
    .write          = rp_wakeup_write,
    .read           = NULL,
};
#endif


static int rp_powerkeylongpress_open(struct inode *inode, struct file *file)
{
       return 0;
}


static ssize_t rp_powerkeylongpress_write(struct file *file, const char *buffer,size_t count, loff_t *data)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0))
static const struct proc_ops rp_powerkeylongpress_ops = {
	.proc_open           = rp_powerkeylongpress_open,
	.proc_write          = rp_powerkeylongpress_write,	
	.proc_read           = NULL,
};
#else
static const struct file_operations rp_powerkeylongpress_ops = {
	.owner          = THIS_MODULE,
    .open           = rp_powerkeylongpress_open,
    .write          = rp_powerkeylongpress_write,
    .read           = NULL,
};
#endif


static int boot_completed_open(struct inode *inode, struct file *file)
{
       return 0;
}


static ssize_t boot_completed_write(struct file *file, const char *buffer,size_t count, loff_t *data)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0))
static const struct proc_ops boot_completed_ops = {
	.proc_open           = boot_completed_open,
	.proc_write          = boot_completed_write,	
	.proc_read           = NULL,
};
#else
static const struct file_operations boot_completed_ops = {
    .owner          = THIS_MODULE,
    .open           = boot_completed_open,
    .write          = boot_completed_write,
    .read           = NULL,
};
#endif
...
static int rp_power_probe(struct platform_device *pdev) {
	...
	static struct proc_dir_entry *root_entry_rp_wakeup;
	static struct proc_dir_entry *root_entry_rp_powerkeylongpress;
	static struct proc_dir_entry *root_entry_boot_completed;
		...
		switch(gpio_data->rp_power_num[gpio_cnt].gpio_function) {
			case GPIO_FUNCTION_INPUT :		/* init input gpio */
				...
				if (ret < 0)
				{
					...
					//gpio_set_value(gpio_data->rp_power_num[gpio_cnt].gpio_num,!gpio_data->rp_power_num[gpio_cnt].action);
  ...
  /* init struct input_dev */ 
       ...
       input_set_capability(gpio_data->input, EV_KEY, KEY_POWER);
       ret = input_register_device(gpio_data->input);
       
       root_entry_rp_wakeup = proc_mkdir("rp_wakeup", NULL);
       proc_create("rp_wakeup_proc", 0666 , root_entry_rp_wakeup , &rp_wakeup_ops);
       
       
       root_entry_rp_powerkeylongpress = proc_mkdir("rp_powerkeylongpress", NULL);
       proc_create("rp_powerkeylongpress_proc", 0666 , root_entry_rp_powerkeylongpress , &rp_powerkeylongpress_ops);
       
       
       root_entry_boot_completed = proc_mkdir("rp_system_flag", NULL);
       proc_create("rp_boot_completed", 0666 , root_entry_boot_completed , &boot_completed_ops);
