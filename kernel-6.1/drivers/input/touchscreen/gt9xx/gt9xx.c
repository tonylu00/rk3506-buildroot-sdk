...
//    printk("ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w);
    if(gpio_is_valid(buzzer_gpio))
    	gpio_set_value_cansleep(buzzer_gpio, 1);
}
...
static void gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
   ...
   // GTP_DEBUG("Touch id[%2d] release!", id);
	...
	// printk("up\n");
    	...
    	if(gpio_is_valid(buzzer_gpio))
            gpio_set_value_cansleep(buzzer_gpio, 0);
    }
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    ret = GTP_GPIO_REQUEST(buzzer_gpio, "GTP BUZZER GPIO");
    if (ret < 0)
    {
        GTP_ERROR("Failed to request GTP BUZZER GPIO:%d, ERRNO:%d",(s32)buzzer_gpio,ret);
        //ret = -ENODEV;
    }else{
...
static void gtp_parse_dt(struct device *dev)
{
	...
	buzzer_gpio  = of_get_named_gpio(np, "buzzer_gpio", 0);
